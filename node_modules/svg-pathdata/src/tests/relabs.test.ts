...
describe('toRel', () => {
  test('should work with M commands', () => {
    ...
    ).toEqual(new SVGPathData('m100,100 m10,-10 m10,-10 m10,-10').encode());
  });

  test('should work with m commands', () => {
    expect(
      new SVGPathData('M-100,100m90,-90M20,20M19,19').toRel().encode(),
    ).toEqual(new SVGPathData('m-100,100 m90,-90 m30,10 m-1,-1').encode());
  });

  test('should work with H commands', () => {
    expect(new SVGPathData('M0,0 H100 H10  H20 H15').toRel().encode()).toEqual(
      new SVGPathData('m0,0 h100 h-90 h10 h-5').encode(),
    );
  });

  test('should work with V commands', () => {
    expect(new SVGPathData('M0,0 V100 V10  V15V20').toRel().encode()).toEqual(
  ...
  test('should work with L commands', () => {
    ...
    ).toEqual(new SVGPathData('m0,0 l100,-100 l-99,100 l2,2 l-1,-1').encode());
  });
  ...
  test('should work with T commands', () => {
    expect(
      new SVGPathData(`M0,0 T-100,100 T-200,200 T-190,210 T-180,220`)
        .toRel()
        .encode(),
    ).toEqual(
  ...
  test('should work with Z commands', () => {
    ...
    ).toEqual(new SVGPathData(`m10,10 h10 v10 z v10 h-10 z`).encode());
  });
});
...
describe('toAbs', () => {
  test('should work with m commands', () => {
    ...
    ).toEqual(new SVGPathData('M-100,100 M10,10 M20,20 M19,19').encode());
  });

  test('should work with h commands', () => {
    expect(new SVGPathData('M0 0 h100 H10 h10 h-5').toAbs().encode()).toEqual(
  ...
  test('should work with t commands', () => {
    expect(
      new SVGPathData('M0 0 t-100,100 t-100,100 t10,10 t10,10')
        .toAbs()
        .encode(),
    ).toEqual(
  ...
  test('should work with v commands', () => {
    expect(new SVGPathData('M0 0 v100 V10 v5 v5').toAbs().encode()).toEqual(
  ...
  test('should work with l commands', () => {
    expect(
      new SVGPathData('M0 0 l100,-100 L1,0 l2,2 l-1,-1').toAbs().encode(),
    ).toEqual(new SVGPathData('M0 0 L100,-100 L1,0 L3,2 L2,1').encode());
  });
