var HFMATH = (() => {
  ...
  function compile(i) {
    ...
    while (j < content.length) {
      ...
      if (digit == " R") {
        ...
        var x = digit.charCodeAt(0) - ordR - xmin;
  ...
  function plan(expr, mode = "math") {
    ...
    if (expr.text == "\\frac") {
      ...
      b.bbox.x = 0;
      b.bbox.y = 0;
      let mw = Math.max(a.bbox.w, b.bbox.w) * s;
      ...
      b.bbox.x = 0;
      b.bbox.y = 0;
      let mw = Math.max(a.bbox.w, b.bbox.w);
      ...
      let dy = 0;
      let mh = 1;
      for (let i2 = 0; i2 < expr.chld.length; i2++) {
        let c = expr.chld[i2];
        let spac = (_b = {
          "\\quad": 2,
          "\\,": 2 * 3 / 18,
      ...
      for (let i2 = 0; i2 < expr.chld.length; i2++) {
        transform(expr.chld[i2], 1, null, -bb.x + (hasLp ? 1.5 : 0), -bb.y);
      }
  ...
  function flatten(expr) {
    function flat(expr2, dx, dy) {
      ...
      if (expr2.bbox) {
        ...
        dy += expr2.bbox.y;
        if (expr2.text == "\\frac") {
          let h = expr2.chld[1].bbox.y - (expr2.chld[0].bbox.y + expr2.chld[0].bbox.h);
          let e = {
            type: "symb",
            mode: expr2.mode,
            text: "\\bar",
          ...
          let h = expr2.chld[0].bbox.y;
          let xx = Math.max(0, expr2.chld[0].bbox.x - expr2.chld[0].bbox.h / 2);
          let e = {
            type: "symb",
            mode: expr2.mode,
            text: "\\sqrt",
          ...
          let w = Math.min(expr2.chld[0].bbox.x, expr2.chld[1].bbox.x);
          let e = {
            type: "symb",
            mode: expr2.mode,
            text: "(",
      ...
      for (var i = 0; i < expr2.chld.length; i++) {
        let f2 = flat(expr2.chld[i], dx, dy);
        ff.push(...f2);
      }
      return ff;
    }
