<template>
  <div class="element-shadow">
    <div class="row">
      <div style="flex: 2;">Enable shadows:</div>
    ...
    <template v-if="hasShadow && shadow">
      <div class="row">
        <div style="flex: 2;">Horizontal shadow:</div>
        <Slider class="slider" :min="-10" :max="10" :step="1" :value="shadow.h"
          @change="value => updateShadow({ h: value as number })" />
      </div>
      <div class="row">
        <div style="flex: 2;">Vertical shadow:</div>
        <Slider class="slider" :min="-10" :max="10" :step="1" :value="shadow.v"
          @change="value => updateShadow({ v: value as number })" />
      </div>
      <div class="row">
        <div style="flex: 2;">Blur distance:</div>
        <Slider class="slider" :min="1" :max="20" :step="1" :value="shadow.blur"
          @change="value => updateShadow({ blur: value as number })" />
      </div>
      <div class="row">
        <div style="flex: 2;">Shadow color:</div>
        <Popover trigger="click">
          <template #content>
            <ColorPicker :modelValue="shadow.color" @update:modelValue="value => updateShadow({ color: value })" />
          </template>
