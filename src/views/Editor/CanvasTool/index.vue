<template>
  <div class="canvas-tool">
    ...
    <div class="add-element-handler">
      ...
      <div class="handler-item group-btn" v-tooltip="'Insert a shape'" :offset="10">
        ...
        <Popover trigger="click" v-model:value="shapeMenuVisible" style="height: 100%;" :offset="10">
          <template #content>
            <PopoverMenuItem center @click="() => { drawCustomShape(); shapeMenuVisible = false }">Free Drawing</PopoverMenuItem>
          </template>
          <IconDown class="arrow" />
        </Popover>
      </div>
      <FileInput @change="files => insertImageElement(files)">
        <IconPicture class="handler-item" v-tooltip="'Insert a picture'" />
      </FileInput>
      ...
      <Popover trigger="click" v-model:value="chartPoolVisible" :offset="10">
        ...
        <IconChartProportion class="handler-item" v-tooltip="'Insert a chart'" />
      </Popover>
    ...
    <div class="right-handler">
      ...
      <Popover trigger="click" v-model:value="canvasScaleVisible">
        <template #content>
          ...
          <PopoverMenuItem center @click="resetCanvas(); canvasScaleVisible = false">Reset Canvas</PopoverMenuItem>
        </template>
        <span class="text">{{canvasScalePercentage}}</span>
      </Popover>
      <IconPlus class="handler-item viewport-size" v-tooltip="'Canvas zoom（Ctrl + =）'" @click="scaleCanvas('+')" />
      <IconFullScreen class="handler-item viewport-size-adaptation" v-tooltip="'Adapt to screen（Ctrl + 0）'" @click="resetCanvas()" />
    </div>
